...
from qqmusicbox.api import QQMUSIC
from qqmusicbox.player import Player
from qqmusicbox.ui import Ui
from qqmusicbox.osdlyrics import show_lyrics_new_process
from qqmusicbox.config import Config
from qqmusicbox.utils import notify
from qqmusicbox.storage import Storage
from qqmusicbox.cache import Cache
...
class Menu(object):
    ...
    def search(self, category):
        ...
        data = self.api.search(keyword, stype=category)
        if not data:
            return data
        # search result: {'cursor_page': 1, 'keyworde': keyword, 'data':result, 'page_size':18, 'total_num':345}
    ...
    def update_alert(self, version):
        ...
        if latest != version and latest != 0:
            ...
            time.sleep(0.5)
            notify(
                "QQMUSIC-MusicBox installed version:"
                + version
                + "\nQQMUSIC-MusicBox latest version:"
                + latest,
                0,
            )
        ...
        while True:
            ...
            # 进入专辑
                ...
                if datatype in ["songs", "fmsongs"]:
                    song_mid = datalist[idx]["song_mid"]
                    album_id = datalist[idx]["album_id"]
                    album_name = datalist[idx]["album_name"]
                elif self.player.playing_flag:
                    song_mid = self.player.playing_id
                    song_info = self.player.songs.get(str(song_mid), {})
                ...
                if album_id:
                    ...
                    for i in range(len(self.datalist)):
                        if self.datalist[i]["song_mid"] == song_mid:
            ...
            # 开始下载
                ...
                cache_thread = threading.Thread(
                    ...
                    args=(s["song_mid"], s["song_name"], s["artist"], s["mp3_url"]),
                )
                cache_thread.start()
    ...
    def dispatch_enter(self, idx):
        ...
        # 该艺术家的热门歌曲
            ...
            if idx == 0:
                self.datatype = "songs"
                songs = QQMUSIC.artists(artist_id)
                self.datalist = QQMUSIC.dig_info(songs, "songs")

            elif idx == 1:
                albums = QQMUSIC.get_artist_album(artist_id)
                self.datatype = "albums"
                self.datalist = QQMUSIC.dig_info(albums, "albums")

        elif datatype == "djchannels":
            radio_id = datalist[idx]["id"]
            programs = QQMUSIC.djprograms(radio_id)
            self.title += " > " + datalist[idx]["name"]
            self.datatype = "songs"
            self.datalist = QQMUSIC.dig_info(programs, "songs")

        # 该专辑包含的歌曲
        elif datatype == "albums":
            album_id = datalist[idx]["album_id"]
            songs = QQMUSIC.album(album_id)
            self.datatype = "songs"
            self.datalist = QQMUSIC.dig_info(songs, "songs")
        ...
        # 精选歌单选项
            ...
            self.datalist = QQMUSIC.dig_info(data["callback"](), self.datatype)
            self.title += " > " + data["title"]

        # 全站置顶歌单包含的歌曲
        elif datatype in ["top_playlists", "playlists"]:
            playlist_id = datalist[idx]["playlist_id"]
            songs = QQMUSIC.playlist_detail(playlist_id)
            self.datatype = "songs"
            self.datalist = QQMUSIC.dig_info(songs, "songs")
        ...
        # 分类精选
            ...
            self.datalist = QQMUSIC.dig_info(data, self.datatype)
            self.title += " > " + data
        ...
        # 某一分类的详情
            ...
            log.error(data)
            self.datalist = QQMUSIC.dig_info(QQMUSIC.top_playlists(data), self.datatype)
            self.title += " > " + data
        ...
        # 歌曲榜单
            ...
            songs = QQMUSIC.top_songlist(idx)

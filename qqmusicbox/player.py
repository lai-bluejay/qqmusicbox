...
# -*- coding: utf-8 -*-
"""
qqmusicbox/player.py was created on 2019/03/31.
...
class Player(object):
    ...
    def current_song(self):
        ...
        if not self.is_index_valid:
            return {}
        song_mid = self.list[self.index]
        return self.songs.get(song_mid, {})

    @property
    def playing_id(self):
        return self.current_song["song_mid"]
    ...
    def add_songs(self, songs):
        for song in songs:
            song_mid = str(song['song_mid'])
            self.info["player_list"].append(song_mid)
            if song_mid in self.songs:
                self.songs[song_mid].update(song)
            else:
                self.songs[song_mid] = song
    ...
    def refresh_urls(self):
        ...
        if songs:
            for song in songs:
                song_mid = str(song['song_mid'])
                if song_mid in self.songs:
                    self.songs[song_mid]["mp3_url"] = song["mp3_url"]
                else:
                    self.songs[song_mid] = song
    ...
    def download_song(self, song_mid, song_name, artist, url):
        def write_path(song_mid, path):
            self.songs[str(song_mid)]["cache"] = path

        self.cache.add(song_mid, song_name, artist, url, write_path)
    ...
    def cache_song(self, song_mid, song_name, artist, song_url):
        def on_exit(song_mid, path):
            self.songs[str(song_mid)]["cache"] = path
            self.cache.enable = False

        self.cache.enable = True
        self.cache.add(song_mid, song_name, artist, song_url, on_exit)
        self.cache.start_download()
